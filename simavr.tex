...
ee contains the internal state of the simulated external \ac{EEPROM}.
...
\ac{MCU}, and initialized. The firmware is then copied into the program memory.
...
attaches a callback function (\lstinline|i2c_eeprom_in_hook|) to the MOSI \ac{IRQ}. This
...
represents the \ac{FIFO} queue of pending interrupts. Note that the \ac{FIFO}
queue deviates from the actual priority-based interrupt behavior of \ac{AVR}
interrupts.
